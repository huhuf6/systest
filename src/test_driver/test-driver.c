...
void add_test_v(char * filename,char *testname,char *testcmdline)
{
	char *buf;
	int fd;
	buf = slurp(filename);
	if (!buf)
		return;
	cJSON* cmdfile = cJSON_Parse(buf);
	cJSON_AddStringToObject(cmdfile,testname,testcmdline);
...
int main(int argc, char **argv)
{
	...
	int c;
	int add_test=0;
	char *add_test_name = NULL;
	char *add_test_cmd =NULL;
	pid_t cpid;
	struct sigaction sa;

	while ((c =
		getopt(argc, argv, "k:AO:Sa:C:QT:d:ef:hl:n:o:pqr:s:t:x:y"))
		       != -1) {
		switch (c) {
		case 'k':	
			add_test=1;
			add_test_name = strdup(optarg);
			add_test_cmd = strdup(optarg);
			break;
	...
	{
		printf("add a test\n");
		add_test_v(filename,add_test_name,add_test_cmd);
		return 0;
	}
	if (panname == NULL) {
		...
		while ((num_active < keep_active) && (starts != 0)) {
			...
			if (sequential)
				if (++c >= coll->cnt)
					c = 0;
			printf("num_active:%d\n",num_active);
			printf("keep_active:%d\n",keep_active);

		}		/* while ((num_active < keep_active) && (starts != 0)) */
			printf("out run child \n");
		if (starts == 0) {
			if (!quiet_mode)
					 ...
					 char **argv)
{
	...
	for (int j = 0; j < cJSON_GetArraySize(cmdfile); j++) {
		cJSON* item = cJSON_GetArrayItem(cmdfile, j);
